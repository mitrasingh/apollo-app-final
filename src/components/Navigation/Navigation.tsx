			...
			<Container className={styles.customContainer}>
				...
				<Navbar.Brand className={`order-lg-last ${styles.brandnav2}`}>
					<OverlayTrigger
						key="bottom"
						placement="bottom"
						overlay={<Tooltip className="fs-6">Edit your profile</Tooltip>}
					>
						<Link to="/editprofile">
							<Image
				...
				>
					<Nav className="mx-auto" onClick={() => setIsNavbarOpen(false)}>
						<Nav.Link eventKey="1" as={Link} to="/home" className="me-1">
							Home
						</Nav.Link>
						<Nav.Link eventKey="2" as={Link} to="/createtask">
							Create Task
						</Nav.Link>
						<Nav.Link eventKey="3" as={Link} to="/topicboard">
							Topic Board
						</Nav.Link>
						<Nav.Link eventKey="4" onClick={handleVisible}>
							Logout
						</Nav.Link>
					</Nav>
				</Navbar.Collapse>
