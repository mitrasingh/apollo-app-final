...
export const TopicCardList = ({ isTopicsRefreshed }) => {
        ...
        const fetchTopics = async () => {
            try {
                ...
                if (topicsCount > 0) {
                    ...
                    const topicsMap = data.docs.map((doc) => ({ // Declared variable for mapping through retrieved docs
                        ...doc.data(),
                        topicId: doc.id
                    }));
    ...
    const handleLoadMore = async () => {
        ...
        try {
            ...
            if (!isDataEmpty) {
                const topics = data.docs.map((doc) => ({ ...doc.data(), topicId: doc.id })); // Declared variable for mapping through retrieved docs
                ...
                setLastTopic(lastTopicDoc);
            } else { // If data is empty (no additional documents/comments) set boolean value isTopicsEmpty to true
                setIsTopicsEmpty(true);
            }
        } catch (error) {
            console.log(`Error: ${error.message}`);
