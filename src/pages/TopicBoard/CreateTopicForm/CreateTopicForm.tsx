...
type CreateTopicFormProps = {
	setIsCreateTopic: React.Dispatch<React.SetStateAction<boolean>>;
	setIsTopicsRefreshed: React.Dispatch<React.SetStateAction<boolean>>;
};

// Props are from TopicBoard.jsx
export const CreateTopicForm = ({
	setIsCreateTopic,
	setIsTopicsRefreshed,
	...
	const handleCreateTopic = async (data: TopicData) => {
		try {
			const timestamp = createTimestamp();
			const dbRef = collection(db, "topics");
			const addTopic = await addDoc(dbRef, {
				// Firestore will auto generate task ID
				title: data.title,
